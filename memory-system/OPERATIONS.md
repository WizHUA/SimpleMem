...
模型必须提供 `/embeddings` 接口，不自动复用 chat endpoint 或 chat API Key。配置一部分字段会拒绝启动；供应商故障、非法向量、维度漂移会明确报错，不会悄悄伪装为语义检索成功。默认每批 32 条输入，全文 SHA256 索引的 LRU 最多保存 256 个向量，避免重复编码相同记忆；所有批次共享超时预算。缓存不持久化，按主体清除数据时清空向量缓存，清理期间已在途的 embedding 请求不能重新填入缓存。
