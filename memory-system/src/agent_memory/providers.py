...
当 query 要求“方案、行动计划、任务指令卡、部署、演练方案、应急处置、实施计划”等完整产物时，
response_intent 必须设为 action_plan，并补充以下字段：
1. problem_breakdown：把用户行动需求拆成可回答的子问题，例如目标、约束、资源、时空边界、角色、
   协同、风险、交付格式；不要把未给出的答案写进去。
2. required_info：列出必须从记忆或近期对话查到的信息槽位。
3. information_gathering：说明每个槽位需要查哪类记忆/证据，缺失时应标“待补充”而不是编造。
4. integration_steps：说明如何把已查到信息整合为方案，例如先确定目标与边界，再分配资源、排序任务、
   建立协同关系、列风险与缺口。
5. action_template：普通工作计划用 general_action_plan；用户明确要“方案与任务指令卡”、作战/桌面推演
   高层结构时用 ops_plan_task_cards；应急协同时用 incident_action_plan；普通问答用 none。
6. action_elements：固定输出要素。ops_plan_task_cards 必须贴近“方案与任务指令卡”Markdown 骨架：
   # 方案名称；## 作战概述；## 兵力部署；作战区域、中心坐标、作战边界；兵力编成与位置表格
   （单位/平台类型/位置/任务角色）；部署阵型；总体概述；## 任务指令卡；每张卡包含任务、任务类型、
   任务目标、时间要求、装备清单、目标分配、协同关系、交战规则/约束、执行要点。
   填充策略必须分三层：已有证据事实直接写入；地形、方向、阶段、协同方式等可推演变量，用“情景假设”
   或“分案”给出山地/海岛/城镇/开阔地等多种方案；兵力、装备、时间、坐标等不能从证据推出的关键事实
   标为“需确认”，并给出具体追问。不要把整份方案大量写成“待补充”。不得虚构兵力、装备、敌情或坐标；
   不得输出现实身份目标、精确打击参数、规避侦查或伤害最大化指令。
