...
开发分支为 `full-forward`，当前后端版本为 `0.3.3`。保留 SimpleMem 式意图规划、短期优先、多视图召回与动态 K 主线；增加自动整理与演化、可恢复维护任务、FTS5/向量内容投影、证据约束的别名关联、分层摘要及动态运行展示。本轮排除 Engine 集成。
...
回答中的来源编号可点击查看原文、记录时间、版本和作用范围。每条回答保存独立的服务端来源快照，刷新或后续更正不会串到另一轮来源；无快照的旧回答明确提示不可用。正文直接回答，只有用户询问依据或确有证据冲突才展开说明。

对话内的灰色运行提示与可折叠汇流图由 NDJSON 服务事件驱动，记录实际抽取、规划、短期与长期召回、证据筛选、生成与复验阶段；“回放”明确使用已有记录，不模拟模型私有思维链。页面内置三组逐步实验引导，示例只填入输入框，发送后才写入。

`MEMORY_MAINTENANCE_INTERVAL=300` 默认每五分钟为本地配置的主体执行维护；设为 `0` 禁用。模型摘要仅为带来源的派生导航视图，不充当新事实。独立运行进程启动该 worker，SDK 宿主需自行调度。

## 验证入口

本轮回答体验的真实模型检查为5/5，原始回答见 [回答与来源验收](memory-system/docs/reports/ANSWER_UX_QUALITY.md)。
...
界面不再要求填写检索上限；系统根据规划深度、信息需求和上下文预算自动选择 K，证据不足时不凑数。展开回答过程，可查看语义、词法、符号三路与短期/长期库的实际交互、分库数量及动态预算；未配置向量服务会明确显示禁用。设计和算法见 [动态检索说明](memory-system/docs/ADAPTIVE_RETRIEVAL.md)。

页眉“切换会话”提供主题/项目搜索；侧栏分隔线可拖动，方向键可微调，双击恢复默认宽度，收起和宽度会记住。

观测操作：完成一次提问后打开“观测”，先运行无缓存基线，再连续两次运行加速查询。实验不追加聊天事件；比较缓存状态、避免的生成调用、生成/检索/总耗时。当前为请求层精确上下文复用、并发合并及上下文缩减，仍执行规划与检索。[本轮真实数据](memory-system/docs/reports/ADAPTIVE_OBSERVATION.md)保留冷/热全部结果，不以单次网络波动宣称统计加速倍数。

## 对象记忆与检索内容

长期页将同主体、同作用范围的互补属性整合成一张对象记忆卡。例如zfc的别名和特性统一显示，展开仍可分别查看证据、版本和执行纠错。后端 `/api/v1/entities` 实时计算整合视图；底层原子事实与检索引用保持独立，不因展示整合而丢失来源，冲突属性不会合成为确定结论。

三路检索详情新增实际查询条件、主体/属性/值、检查内容、匹配原因和入选状态。每个通道/记忆层最多保存20条候选预览，明确显示截断；内容只来自授权且适用的候选。旧回答无法补造当时未保存的候选，新提问即可获得完整新契约。
